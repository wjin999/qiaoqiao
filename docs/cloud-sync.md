...
新项目按文件名顺序执行两个迁移。大金毛与黑白猫继续使用 `202609200002_companion_preferences.sql` 已允许的设置键，已执行过迁移的项目无需修改数据库。旧版角色的记录仍兼容，但不计入新角色的成长；选择旧角色的设备更新后默认显示大金毛，保留原来的显示和动画开关。

每完成并保存一句，当前伙伴成长 +1，四种评分奖励相同。成长从逐句记录的 `petId` 计算，按记录 ID 去重，包含在进度备份中；伙伴选择与开关同学习进度一起同步。
...
- 登录用户完成一组记忆复习或自由跟打后，在最后一条记录保存成功、进入结算页时自动同步一次，包含学习进度、宠物成长与设置。逐句输入、未完成就返回主页、登录、重新联网、切回页面不会自动触发。仍保留页头「同步」和账号窗口「立即同步」。离线或失败时保留本地队列并提示手动重试，不循环重试；下一组完成会再次尝试。同步中暂停开始下一组，防止练习时应用云端变化。换设备前确认已同步，再在新设备登录并同步。
