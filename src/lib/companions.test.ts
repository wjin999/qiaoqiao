...
import { companionPreferences, equippedOutfit, growthStage, petGrowth } from './companions'
...
describe('companion growth', () => {
  it('preserves independent outfits across JSON sync and validates ownership and unlock thresholds', () => {
    const prefs = companionPreferences(JSON.stringify({ petId: 'golden', outfits: { golden: 'bomber', tuxedo: 'space' } }))
    expect(equippedOutfit(prefs, 'golden', 29)).toBe('natural')
    expect(equippedOutfit(prefs, 'golden', 30)).toBe('bomber')
    expect(equippedOutfit(prefs, 'tuxedo', 149)).toBe('natural')
    expect(equippedOutfit(prefs, 'tuxedo', 150)).toBe('space')
    expect(companionPreferences(JSON.stringify(prefs))).toEqual(prefs)
    expect(companionPreferences('{"outfits":{"golden":"space","tuxedo":"https://bad"}}').outfits).toEqual({ golden: 'natural', tuxedo: 'natural' })
    expect(equippedOutfit(companionPreferences('{}'), 'golden', 9999)).toBe('natural')
  })
