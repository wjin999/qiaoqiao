...
it('advances drawn frames, freezes in background, then finishes a single page turn', () => {
  render()
  act(() => container.querySelector('.pet-turn-frame img')!.dispatchEvent(new Event('load')))
  click('翻一页')
  const frame = () => Number(container.querySelector('.pet-turn-frame [data-frame]')!.getAttribute('data-frame'))
  ...
  vi.spyOn(document, 'hidden', 'get').mockReturnValue(false)
  act(() => document.dispatchEvent(new Event('visibilitychange')))
  for (let i = 0; i < 5; i++) act(() => vi.advanceTimersByTime(180))
  expect(frame()).toBe(7)
  act(() => vi.advanceTimersByTime(1000))
  expect(frame()).toBe(7)
})
